...
public:
                  ...
                  std::vector<double>& lower)
  {
    ...
    if (trainingParameters_.treeType == 3)
      {
        pStatistics.SetBound(upper, lower);
      }
    ...
    if (trainingParameters_.treeType == 3)
      {
        upperBefore = upper;
        lowerBefore = lower;
        upper[bestClassifier.Axis()] = bestClassifier.threshold_;
      }
    ...
    if (trainingParameters_.treeType == 3)
      {
        upper = upperBefore;
        lower = lowerBefore;
        lower[bestClassifier.Axis()] = bestClassifier.threshold_;
      }
  ...
  void Training(DecisionTreeT& tree)
  {
    ...
    if (trainingParameters_.treeType == 3)
      {
        upper.resize(trainingData_.dataDim_);
        lower.resize(trainingData_.dataDim_);
        for(index_t i = 0; i < trainingData_.dataDim_; ++i)
          {
            upper[i] = INFINITY;
            lower[i] = -INFINITY;
          }
